...
for(let i=0;i<seatrow1;i++){
  ...
  for(let j=0;j<seatcol1;j++){
    currentrow1.innerHTML = currentrow1.innerHTML + `<a class='seat1 pointer' id="r-${i}-${j}">${j+1}</a>`;
  }
}
...
for(let i=0;i<seatrow2;i++){
  ...
  for(let j=0;j<seatcol2;j++){
    currentrow2.innerHTML = currentrow2.innerHTML + `<a class='seat2 pointer' id="r-${i}-${j}">${j+1}</a>`;
  }
}
...
for(let i=0;i<seatrow3;i++){
  ...
  for(let j=0;j<seatcol1;j++){
    currentrow3.innerHTML = currentrow3.innerHTML + `<a class='seat3 pointer' id="r-${i}-${j}">${j+1}</a>`;
  }
}
...
//   ticketCount = Number(localStorage.getItem('ticketCount'));
